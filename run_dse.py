...
Usage example:
  ...
  # Single-objective BO (scalarized), compare with random baseline
  python run_dse.py \\
    --algos bo_gp random \\
    --seeds 42 43 44 \\
...
Comparison note:
  ...
  - Within multi-track: nsga2 vs mobo vs random are compared via Hypervolume.
  - Within single-track: bo_gp vs random are compared via best scalarized objective.
...
from dse.metrics import compute_reference_point, hypervolume_3d, pareto_indices
from dse.output import DSERunResult, RunConfig, print_report, write_all, write_comparison


# ---------------------------------------------------------------------------
# Per-trial worker (runs in a subprocess)
# ---------------------------------------------------------------------------
...
def _run_trial(algo: str, seed: int, run_cfg_dict: Dict[str, Any], output_dir: str) -> str:
    ...
    # Reconstruct RunConfig from serialisable dict
...
# ---------------------------------------------------------------------------
# Post-run: apply global hypervolume reference
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Result loader (for post-hoc comparison without re-running)
# ---------------------------------------------------------------------------
...
def load_results_from_dir(output_root: str) -> List[DSERunResult]:
    ...
    for trial_dir in sorted(root.iterdir()):
        ...
        # Reconstruct minimal RunConfig from result.json
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    # --- Algorithm and seed selection ---
    p.add_argument(
        "--algos", nargs="+", default=["nsga2", "mobo"],
        choices=["bo_gp", "nsga2", "mobo", "random"],
        help="Algorithms to run. Multi-track: nsga2/mobo/random. Single-track: bo_gp.",
    )
    ...
    # --- Compare-only mode (load existing results and recompute comparison) ---
...
def main() -> None:
    ...
    if args.compare_only:
        ...
        # Re-write result.json with updated HV
        for r in results:
    ...
    # --- Build trial list ---
    trials: List[Tuple[str, int, str]] = []  # (algo, seed, trial_output_dir)
    ...
    algo_kwargs_map = {"bo_gp": bo_kwargs, "nsga2": nsga2_kwargs, "mobo": {}, "random": {}}
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as pool:
        ...
        for future in as_completed(future_to_trial):
            ...
            done_count += 1
            try:
                result_json_path = future.result()
                # Load the written result to accumulate
                loaded = load_results_from_dir(str(Path(trial_dir).parent))
                # Find the specific trial result
                for r in loaded:
    ...
    # --- Compute global hypervolume reference and update results ---
    ...
    # Re-write result.json with HV values
